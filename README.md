...
同名裸模型名每一轮会按 Console → Build → Web 扫完全部合格渠道（`maxAttempts` 控制轮数，每轮每渠道取一个账号）；带 `Build/`、`Web/`、`Console/` 前缀的名称锁定单渠道；会话粘滞与 `previous_response_id` 归属仍会钉住原 Provider。
...
### Egress and Proxy Configuration
