...
func TestGatewayErrorHidesUpstreamCredentialStatus(t *testing.T) {
	...
	openAIRecorder := httptest.NewRecorder()
	openAIRouter.ServeHTTP(openAIRecorder, httptest.NewRequest(http.MethodGet, "/", nil))
	if openAIRecorder.Code != http.StatusServiceUnavailable || !strings.Contains(openAIRecorder.Body.String(), `"code":"permission-denied"`) || !strings.Contains(openAIRecorder.Body.String(), "Upstream service unavailable; chat endpoint access denied") || strings.Contains(openAIRecorder.Body.String(), "secret") || strings.Contains(openAIRecorder.Body.String(), "The upstream service rejected the request") {
		t.Fatalf("OpenAI status=%d body=%s", openAIRecorder.Code, openAIRecorder.Body.String())
	}
...
func TestDirectUpstreamCredentialResponsesAreRewritten(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantCode == "permission-denied" && !strings.Contains(recorder.Body.String(), "Upstream service unavailable; chat endpoint access denied") {
...
func TestImageEditAcceptsOfficialJSONShape(t *testing.T) {
	...
	if invalidResolutionRecorder.Code != http.StatusBadRequest || !strings.Contains(invalidResolutionRecorder.Body.String(), "resolution must be 1k or 2k") {
	...
	if invalidCountRecorder.Code != http.StatusBadRequest || !strings.Contains(invalidCountRecorder.Body.String(), "n must be between 1 and 10") {
