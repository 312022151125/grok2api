...
func (h *Handler) generateVideo(c *gin.Context) {
	...
	if len(inputs) > mediadomain.MaxInputImages {
		writeOpenAIError(c, http.StatusBadRequest, "invalid_request", fmt.Sprintf("image and reference_images combined cannot exceed %d", mediadomain.MaxInputImages))
		return
	}
...
func (value responseUsageDTO) toGatewayUsage(responseModel string) gateway.Usage {
	...
	// Unified cache-hit handling for Responses, Chat Completions, and Anthropic Messages
	cached := value.InputTokensDetails.CachedTokens
	if cached == 0 {
		cached = value.PromptTokensDetails.CachedTokens
	}
	if cached == 0 {
		// Anthropic cache hits use cache_read_input_tokens; cache creation is write cost, not read discount
...
func credentialErrorMessage(code string) string {
	if code == "permission-denied" {
		return "Upstream service unavailable; chat endpoint access denied"
	}
