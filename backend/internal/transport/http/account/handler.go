...
func (h *Handler) exportSelectedCredentials(c *gin.Context) {
	...
	providerValue := accountdomain.Provider(request.Provider)
	result, err := h.service.ExportSelectedCredentials(c.Request.Context(), providerValue, ids)
	if err != nil {
...
func (h *Handler) validateProviderIDs(c *gin.Context, ids []uint64, providerValue string) bool {
	...
	if err != nil {
		h.writeServiceError(c, "accountPoolValidationFailed", err, http.StatusInternalServerError, "Failed to validate account pool")
		return false
	}
	if !valid {
