// Package cliproxyexport contains types and helpers for producing CLIProxyAPI
// auth-dir exports. It is intentionally free of dependencies on the cli adapter
// package so that application/account can use it without creating an import cycle.
package cliproxyexport
...
const (
	...
	cliproxyTokenURL     = "https://auth.x.ai/oauth2/token"
...
func buildCLIProxyAuthDocument(accessToken, refreshToken, email, userID string, enabled bool, priority int, expiresAt time.Time, lastRefreshAt *time.Time, now time.Time) cliproxyAuthDocument {
	...
	doc := cliproxyAuthDocument{
		...
		TokenEndpoint: cliproxyTokenURL,
