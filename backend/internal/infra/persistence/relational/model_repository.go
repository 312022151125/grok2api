...
const (
	// Failover candidate order: Console → Build → Web (bare model names).
	modelProviderPriorityExpression = "CASE model_routes.provider WHEN 'grok_console' THEN 0 WHEN 'grok_build' THEN 1 WHEN 'grok_web' THEN 2 ELSE 3 END"
...
// 返回顺序遵循 Console → Build → Web 的稳定 Provider 故障切换顺序。
