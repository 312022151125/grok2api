...
func (s *Service) applyInput(value domain.Node, input Input, create bool) (domain.Node, error) {
	...
	if !validListScope(input.Scope) || input.Scope == "" {
		return domain.Node{}, fmt.Errorf("%w: scope must be grok_build, grok_web, grok_console, grok_web_asset, or grok_console_asset", ErrInvalidInput)
	}
...
func NormalizeProxyURL(value string) (string, error) {
	...
	if tunnelproxy.IsSupportedScheme(scheme) {
		if hasAccountPlaceholder {
			return "", errors.New("tunnel proxies do not support the {account} placeholder")
		}
	...
	default:
		return "", errors.New("proxy URL scheme must be HTTP, HTTPS, SOCKS4, SOCKS5, Trojan, VLESS, SS, or VMess")
	}
