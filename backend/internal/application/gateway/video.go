...
func (s *Service) runVideoJob(parent context.Context, job media.Job, route model.Route) {
	...
	result, err := adapter.GenerateVideo(ctx, provider.VideoRequest{
		Credential: lease.Credential, Billing: lease.Billing, JobID: job.ID, Model: route.UpstreamModel, Prompt: job.Prompt, Duration: job.Seconds, AspectRatio: job.Size, Resolution: job.Quality,
