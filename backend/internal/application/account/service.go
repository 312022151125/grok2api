...
var (
	...
	ErrConflict            = errors.New("Account operation conflicts with current state")
	ErrAccountPoolMismatch = errors.New("Batch operation includes accounts that do not belong to the current pool")
)

var ErrCredentialRefreshPermanent = errors.New("OAuth refresh token is permanently invalid")
var errQuotaRefreshBusy = errors.New("quota sync is already being executed by another instance")
...
// ExportSelectedCredentials 导出管理端明确选择且属于指定 Provider 的账号，
// 作为 POST /accounts/export 的服务端入口；保留 ExportProviderCredentialsByIDs
// 以兼容内部调用方。
func (s *Service) ExportSelectedCredentials(ctx context.Context, providerValue accountdomain.Provider, ids []uint64) (ExportResult, error) {
	return s.ExportProviderCredentialsByIDs(ctx, providerValue, ids)
}
